...
- GitHub Actions run status (hiện đang fail do mismatch Node version)
- GitHub Actions secrets config (Cloudflare)
- `public/robots.txt`
...
|---------|------|----------------|-----------|--------------------|
| P0 | Chạy build test + fix lỗi | Xác nhận site chạy được trước khi đổ thêm asset | Repo hiện tại | DONE (npm run build pass locally) |
| P0 | Tạo GitHub Actions CI/CD cho Cloudflare Pages | Khóa đường deploy ổn định, tránh deploy tay | Build logic cơ bản rõ | DONE (`.github/workflows/deploy.yml` created) |
| P0 | Sửa Node version & kiểm tra Cloudflare secrets | Đảm bảo CI/CD thực sự hoạt động trên GitHub | Đã có workflow file | Workflow chạy pass, secrets được cấu hình đúng |
...
1. fix Node version in `.github/workflows/deploy.yml` (DONE)
2. verify GitHub Actions run status (needs push)
3. verify Cloudflare secrets in GitHub
